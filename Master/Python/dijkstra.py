...
def Dijkstra(G, start, end=None):
    ...
    for v in Q:
        ...
        #print(v)
        #print(G)
        # if G[v] is empty: return not yet schedulable
        if v not in G:
            raise ValueError("Dijkstra: {} not in graph G ({} -> {})".format(v, start, end))
        for w in G[v]:
            vwLength = D[v] + G[v][w]
            if w in D:
                if vwLength < D[w]:
...
def shortestPath(G, start, end):
    ...
    #try:
    D, P = Dijkstra(G, start, end) # catch error
    #except ValueError as e: raise
    D = D[end]  # cost of path
    Path = []
    while 1:
        Path.append(end)
        if end == start:
